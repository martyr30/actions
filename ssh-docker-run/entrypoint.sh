...
function main() {
  ...
  ls -a
  scp -f -o StrictHostKeyChecking=no ./ssh-docker-run/run.sh $INPUT_USER@$INPUT_HOST:/home/$INPUT_USER/
  ssh -o StrictHostKeyChecking=no -A -tt $INPUT_USER@$INPUT_HOST "./run.sh $INPUT_IAM"
